...
class SingleWord:
    ...
    def start_screen(self) -> str: 
        ...
        while running:
            ...
            self.screen.blit(input_text_surface,(self.rect_x + input_text_mar_x, self.rect_y + input_text_mar_y))         
    ...
    def main_screen(self):
        ...
        while running:
            ...
            if self.elapsed_time >=  self.time_limit:
                self.wpm = (self.correct_chars / 5) / (self.elapsed_time / 60)
                running = False
            ...
            self.screen.fill(self.BG)
            self.screen.blit(text_surface,(self.rect_x + text_mar_x, self.rect_y + text_mar_y))         
            self.screen.blit(word_surface,(self.rect_x + margin_x, self.rect_y + margin_y - 250) )
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    if event.key == pg.K_ESCAPE:
                        ...
                        self.main_screen()
            self.screen.fill(self.BG)
            self.screen.blit(wpm_surface, (self.rect_x + wpm_mar_x, self.rect_y + wpm_mar_y))
            self.clock.tick(self.FPS)
            pg.display.flip()
